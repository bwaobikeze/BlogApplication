...
function ViewPost() {
  ...
  useEffect(() => {
    axios
      .get(`https://us-central1-blogapplication-431707.cloudfunctions.net/app/post/${id}`)
      .then((response) => {
  ...
  const handleLike = () => {
    axios
      .post(`https://us-central1-blogapplication-431707.cloudfunctions.net/app/post/${id}`)
      .then((response) => {
        setLike(response.data.totalLikes);
        setLiked(true);
      })
